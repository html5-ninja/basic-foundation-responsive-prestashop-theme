    ...
    <ul class="product_list">
        {foreach from=$products item=product name=products}
            <li class="ajax_block_product {if $smarty.foreach.products.first}first_item{elseif $smarty.foreach.products.last}last_item{/if} {if $smarty.foreach.products.index % 2}alternate_item{else}item{/if} clearfix">
                <div class="row">
                    <div class="left_block  one columns">
                    ...
                    <div class="eight columns">
                        <div class="row">
                            <div class="three columns">
                                <a href="{$product.link|escape:'htmlall':'UTF-8'}" class="product_img_link th" title="{$product.name|escape:'htmlall':'UTF-8'}">
                                ...
                                {if isset($product.new) && $product.new == 1}<span class="new round label">{l s='New'}</span>{/if}
                            </a>
                        </div>
                        <div class="nine columns">
                            <h2><a href="{$product.link|escape:'htmlall':'UTF-8'}" title="{$product.name|escape:'htmlall':'UTF-8'}">{$product.name|escape:'htmlall':'UTF-8'|truncate:35:'...'}</a></h2>
                            <p class="lead">{$product.description_short|strip_tags:'UTF-8'|truncate:360:'...'}</p>
                        </div>
                    </div>
                </div>
