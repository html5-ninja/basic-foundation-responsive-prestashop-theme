{if $block == 1}
    ...
    {foreach from=$cms_titles key=cms_key item=cms_title}
        <div id="informations_block_left_{$cms_key}">
            <h5><a href="{$cms_title.category_link}">{if !empty($cms_title.name)}{$cms_title.name}{else}{$cms_title.category_name}{/if}</a></h5>
            <ul class="block_content panel">
            ...
            {/foreach}
                ...
                {else}
                    <!-- MODULE Block footer -->
                    <div class="three columns hide-for-small" id="block_various_links_footer">
                        <h6>{l s='Information' mod='blockcms'}</h6>
                        <ul>
