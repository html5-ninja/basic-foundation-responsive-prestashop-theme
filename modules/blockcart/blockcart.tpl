...
<!-- MODULE Block cart -->
<div id="cart_block_content" class="reveal-modal [expand, xlarge, large, medium, small]">
    <div id="cart_block">
        <h2>
        ...
        <div class="block_content">
            ...
            <div id="cart_block_list" class="responsive {if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)}expanded{else}collapsed{/if}">
                {if $products}
                    <dl class="products">
    ...
    <ul class="cart_block_customizations" id="customization_{$productId}_{$productAttributeId}">
        {foreach from=$customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] key='id_customization' item='customization' name='customizations'}
            ...
            {/if}
                ...
                <a href="{$link->getPageLink("$order_process", true)}" id="button_order_cart" class="button right small {if $order_process == 'order-opc'}_large{/if}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">{l s='Check out' mod='blockcart'}</a>
            </p>
        </div>
    </div>
</div>
